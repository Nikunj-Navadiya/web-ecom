...
const Orders = ({ token }) => {
  ...
  const fetchAllOrders = async () => {
    ...
    try {
      const respons = await axios.post(backendUrl + '/api/order/list', {}, { headers: { token } })
  ...
  const statusHandler = async (event, orderId) => {
    try {
      const reaponse = await axios.post(backendUrl + '/api/order/status', { orderId, status: event.target.value }, { headers: { token } });
